...
*The whole workflow is almost done by Snakemake, as Snakemake may conflict with dependencies in several Conda environments, we manage Snakemake at the system level (not inside Conda). Install it via APT:
...
```
In our workflow, snakemake version is 7.32.4.
...
* **Environment and scripts**
  ...
  The whole environment can be accessed in env/model_training.yaml in this repository. The environment can be directly built by
    ...
    &emsp;&emsp;1) LuCaP_final_feature_matrix.tsv, lpWGS_final_feature_matrix.tsv, clinic_final_feature_matrix, healthy_final_features.tsv generated in 4)Feature substraction part;

    &emsp;&emsp;2) LuCaP_TF.txt, lpWGS_TF.txt, clinic_TF generated by scripts/collect_ichor_tf.py.

    &emsp;&emsp;3) LuCaP_subtypes.tsv generated from https://github.com/GavinHaLab/CRPCSubtypingPaper/blob/main/Data/LuCaP_subtypes.txt
...
* **Environment and scripts**
  ...
  &emsp;&emsp;1) LuCaP_TF.txt, LuCaP_subtypes.tsv directly in the TF_analysis directory;

  &emsp;&emsp;2) The griffin results on 381 transcription factor binding sites should be put in a directory. Here take /mnt/f/Project_LuCap/Griffin_results/griffin_nucleosome_profiling/ as an example.
...
* **Environment and scripts**
  ...
  &emsp;&emsp;1) clinic_predicted_combined_LLR_softTF_predictions.tsv, lpWGS_predicted_combined_LLR_softTF_predictions.tsv which contains prediction score of lpWGS and clinical data generated from 5) Model training.

  &emsp;&emsp;2) lpWGS_clinical_ubc.txt. A clinical file contains survival period and tumor fraction of 64 lpWGS samples

  &emsp;&emsp;3) clinic_TF.txt
