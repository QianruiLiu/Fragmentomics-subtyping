...
# Fragmentomics Subtyping (ARPC vs NEPC) — Reproducible Workflow

> End‑to‑end pipeline to extract multi‑family cfDNA fragmentomics (FinaleToolkit + Griffin), train NEPC vs ARPC classifiers, perform TF‑level analyses, and project survival from lpWGS. This README mirrors the **sectioning and narrative style** of thorough academic project READMEs (intro, tools, pipelines, step‑by‑step commands, reproducibility, troubleshooting) and is meant to be self‑contained for others to reproduce.

---

## 0) Project Links & At‑a‑Glance

* **This repository:** <this‑repo‑url>
* **FinaleToolkit Snakemake workflow (external):** [https://github.com/epifluidlab/finaletoolkit_workflow](https://github.com/epifluidlab/finaletoolkit_workflow)
* **Griffin / coverage profiling resources:** <link to Griffin repo/docs>
* **Paper/report:** `docs/report.pdf` or `docs/report.docx`
* **Contact:** <name · email>
...
# edit paths, references, accession IDs

# 4) Run pipelines
snakemake -s workflow/Snakefile -c 16 --use-conda
# FinaleToolkit (external)
snakemake -s ../finaletoolkit_workflow/Snakefile -c 16 --use-conda

# 5) Analyses in R (each script produces figures)
...
.
├─ config/
│  ├─ config.example.yaml        # main config; copy to config.yaml and edit
│  └─ samples/                   # sample sheets (lucap.tsv, lpwgs64.tsv, clinical.tsv)
├─ workflow/
│  ├─ Snakefile                  # orchestrates this repo’s rules
│  └─ rules/                     # modular rules (griffin, merges, qc, tables)
├─ scripts/                      # bash/python helpers (called by rules or manually)
│  ├─ align_markdup.sh           # bwa-mem2 → sort → mark/remove dup → index
│  ├─ ichor_tf_collect.py        # parse ichorCNA params → sample \t TF
│  ├─ fraglen_features.py        # fragment-length histograms/summaries → features
│  ├─ motif_kmer_features.py     # end-motif fractions + entropy
│  ├─ profile_metrics.py         # tidy meta-profiles → WPS/cleavage metrics
│  ├─ merge_features.py          # merge AD/NE multi-family features
│  ├─ join_final_griffin.py      # left-join FinaleToolkit + Griffin wide tables
│  └─ griffin_sitewise.py        # explode site-wise coverage metrics
├─ R/
│  ├─ 01_ml_classifier.R         # glmnet/tidymodels + viz
│  ├─ 02_tf_analysis.R           # sAUC_dir ranking + heatmaps + profile plots
│  └─ 03_survival_projection.R   # ctF-aware calibration + Cox + KM
├─ envs/                         # conda env specs (finaletoolkit.yml, griffin.yml, rstats.yml)
├─ tables/                       # intermediate TSVs (features, preds, survival projections)
├─ figs/                         # figures generated by R scripts
└─ docs/                         # report, methods, figure legends
```
